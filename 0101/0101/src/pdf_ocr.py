...
def process_pdf(pdf_path: Path, pages: list[int] | None):
    ...
    candidates = [d for d in OUT_DIR.iterdir() if d.is_dir() and pdf_path.stem in d.name]
    if candidates:
        base = candidates[0]
    else:
        base = OUT_DIR / pdf_path.stem
    ...
    if not base.exists():
        ...
        return
    # discover pages
    pages_dirs = sorted([p for p in base.iterdir() if p.is_dir() and p.name.startswith('page')])
