...
def list_voices(engine):
    ...
    for v in voices:
        print(
            f"- id: {v.id}, name: {v.name}, gender: {getattr(v, 'gender', 'unknown')}"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Start page (default: 1 or bookmark)",
    )
    parser.add_argument(
        "--end-page", type=int, default=None, help="End page (default: last)"
    )
    parser.add_argument(
        "--voice", type=str, default=None, help="Voice id or gender (optional)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--female-slow",
        action="store_true",
        help="Use a slower female voice for output",
    )
    parser.add_argument(
        "--list-voices", action="store_true", help="List available voices and exit"
    )
    parser.add_argument(
        "--export-mp3", type=str, help="Export selected pages to MP3 (requires gTTS)"
    )
    parser.add_argument(
        "--by-chapter",
        action="store_true",
        help="Read by chapter/section markers instead of pages",
    )
    parser.add_argument(
        "--bookmark", action="store_true", help="Resume from last bookmark (if any)"
    )
    parser.add_argument(
        "--interactive",
        action="store_true",
        help="Interactive mode: control playback in real time",
    )
    parser.add_argument(
        "--gui", action="store_true", help="(Stub) Launch GUI (not implemented)"
    )
    ...
    if start > end or start < 1 or end > total_pages:
        print(
            f"Invalid page range: {start}-{end} (book has {total_pages} pages)",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if args.female_slow:
        ...
        if args.voice:
            ...
            for v in voices:
                if (
                    args.voice.lower() in v.id.lower()
                    or args.voice.lower() in v.name.lower()
                ):
