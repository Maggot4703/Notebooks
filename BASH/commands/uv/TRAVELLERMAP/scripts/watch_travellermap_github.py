...
def check_github_releases(repo="inexorabletash/travellermap"):
    ...
    try:
        ...
        data = resp.json()
        print(
            f"Latest release for {repo}: {data.get('tag_name', 'unknown')} ({data.get('name', '')})"
        )
...
def check_github_commits(repo="inexorabletash/travellermap"):
    ...
    try:
        ...
        if data:
            latest = data[0]
            print(
                f"Latest commit: {latest['sha'][:7]} by {latest['commit']['author']['name']} on {latest['commit']['author']['date']}"
            )
