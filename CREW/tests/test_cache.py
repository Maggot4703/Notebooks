...
class TestCache(unittest.TestCase):
    ...
    def test_cache_unwritable_dir(self):
        ...
        try:
            cache = Cache(cache_dir=unwritable_dir)
            # Should still initialize, but saving should fail
            cache.set("k", "v")
            try:
                cache.save()
            except Exception:
    ...
    def test_cache_config_error(self):
    ...
    def test_cache_invalid_json_load(self):
        ...
        # Place bad file in cache dir
        shutil.copy(bad_file.name, os.path.join(self.test_cache_dir, "cache.json"))
        try:
            cache.load()
        except Exception:
            pass
