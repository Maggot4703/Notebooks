...
def drop_abnormal_ym(
    data: pd.DataFrame, min_ym_bad_sample: int = 1, min_ym_sample: int = 500
) -> tuple:
    """Filter abnormal months - overall statistics, not by organization"""
    stat = (
        data.groupby("new_date_ym")
        .agg(bad_cnt=("new_target", "sum"), total=("new_target", "count"))
        .reset_index()
    )

    abnormal = stat[
        (stat["bad_cnt"] < min_ym_bad_sample) | (stat["total"] < min_ym_sample)
    ]
    abnormal = abnormal.rename(columns={"new_date_ym": "年月"})
    abnormal["去除条件"] = abnormal.apply(
        lambda x: (
            f'bad sample count {x["bad_cnt"]} less than {min_ym_bad_sample}'
            if x["bad_cnt"] < min_ym_bad_sample
            else f'total sample count {x["total"]} less than {min_ym_sample}'
        ),
        axis=1,
    )
...
def drop_highmiss_features(
    data: pd.DataFrame, miss_channel: pd.DataFrame, threshold: float = 0.6
) -> tuple:
    ...
    high_miss["去除条件"] = high_miss.apply(
        lambda x: f'overall missing rate is {x["缺失率"]:.4f}, exceeds threshold {threshold}',
        axis=1,
    )
...
def drop_lowiv_features(
    data: pd.DataFrame,
    features: List[str],
    overall_iv_threshold: float = 0.05,
    org_iv_threshold: float = 0.02,
    max_org_threshold: int = 8,
    n_jobs: int = 4,
) -> tuple:
    ...
    print(
        f"   IV calculation: feature count={len(features)}, organization count={len(orgs)}"
    )
    ...
    if len(iv_overall) == 0:
        print(
            "   Warning: Overall IV calculation result is empty, returning empty table"
        )
        return (
            data,
            pd.DataFrame(columns=["变量", "IV值", "机构", "类型"]),
            pd.DataFrame(columns=["变量", "整体IV", "低IV机构数", "处理原因"]),
        )
    ...
    iv_by_org = (
        pd.concat(iv_by_org, ignore_index=True)
        if iv_by_org
        else pd.DataFrame(columns=["变量", "IV值", "机构"])
    )
    ...
    iv_overall_low = iv_overall[iv_overall["IV值"] < overall_iv_threshold][
        "变量"
    ].tolist()
    ...
    if len(iv_by_org) > 0:
        iv_by_org_low = (
            iv_by_org[iv_by_org["IV值"] < org_iv_threshold]
            .groupby("变量")
            .size()
            .reset_index()
        )
    ...
    if len(iv_by_org) > 0:
        for var in iv_by_org["变量"].unique():
            var_orgs = iv_by_org[
                (iv_by_org["变量"] == var) & (iv_by_org["IV值"] < org_iv_threshold)
            ]["机构"].tolist()
    ...
    if len(iv_overall) > 0:
        print(
            f"   Overall IV statistics: min={iv_overall['IV值'].min():.4f}, max={iv_overall['IV值'].max():.4f}, median={iv_overall['IV值'].median():.4f}"
        )
        print(
            f"   Number of features with overall IV less than {overall_iv_threshold}: {(iv_overall['IV值'] < overall_iv_threshold).sum()}/{len(iv_overall)}"
        )

    if len(iv_by_org_low) > 0:
        print(
            f"   Statistics of features with organization IV less than {org_iv_threshold}:"
        )
        print(
            f"     Maximum low IV organization count: {iv_by_org_low['低IV机构数'].max()}"
        )
        print(
            f"     Number of features with low IV organization count greater than or equal to {max_org_threshold}: {(iv_by_org_low['低IV机构数'] >= max_org_threshold).sum()}/{len(iv_by_org_low)}"
        )
    ...
    for var in features:
        ...
        if len(var_overall_iv) > 0 and var_overall_iv[0] < overall_iv_threshold:
            reasons.append(
                f"overall IV {var_overall_iv[0]:.4f} less than threshold {overall_iv_threshold}"
            )
        ...
        if (
            len(var_org_low) > 0
            and var_org_low["低IV机构数"].values[0] >= max_org_threshold
        ):
            reasons.append(
                f'IV less than threshold {org_iv_threshold} in {var_org_low["低IV机构数"].values[0]} organizations'
            )
...
def drop_highcorr_features(
    ...
    if gain_dict:
        ...
        current_gain_dict = {
            k: v for k, v in gain_dict.items() if k in current_features
        }
        if current_gain_dict:
            sorted_features = sorted(
                current_gain_dict.keys(),
                key=lambda x: current_gain_dict[x],
                reverse=True,
            )
    ...
    while True:
        ...
        gain_str = ",".join(
            [f"{v}(gain={gain_dict.get(v, 0):.2f})" for v, c in related_vars]
        )
        dropped_info.append(
            {
                "变量": to_drop,
        ...
        print(
            f"   Dropped feature: {to_drop} (original gain={gain_dict.get(to_drop, 0):.2f})"
        )
...
def drop_highnoise_features(
    data: pd.DataFrame,
    features: List[str],
    n_estimators: int = 100,
    max_depth: int = 5,
    gain_threshold: float = 50,
) -> tuple:
    ...
    for i in range(2):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X, Y, test_size=0.3, random_state=random_n
        )
        ...
        X_train_, X_test_, y_train_, y_test_ = train_test_split(
            X, Y_permuted, test_size=0.3, random_state=random_n
        )
        ...
        imp_perm = pd.DataFrame(
            {
                ...
                "gain": clf_permuted.booster_.feature_importance(
    ...
    imp_perm_avg = (
        pd.concat(results_permuted).groupby("feature")["gain"].mean().reset_index()
    )

    comparison = imp_real_avg.merge(
        imp_perm_avg, on="feature", suffixes=("_real", "_perm")
    )
    ...
    noise_features = comparison[comparison["gain_diff"] < gain_threshold][
        "feature"
    ].tolist()
    ...
    dropped_info["状态"] = dropped_info.apply(
        lambda x: (
            "去除"
            if np.abs(x["原始gain"] - x["反转后gain"]) < gain_threshold
            else "保留"
        ),
        axis=1,
    )
...
def drop_highpsi_features(
    data: pd.DataFrame,
    features: List[str],
    psi_threshold: float = 0.1,
    max_months_ratio: float = 1 / 3,
    max_orgs: int = 4,
    min_sample_per_month: int = 100,
    n_jobs: int = 4,
) -> tuple:
    ...
    results = Parallel(n_jobs=n_jobs, verbose=0)(
        delayed(_calc_single_psi)(task) for task in tasks
    )
    ...
    if len(psi_df) == 0:
        return (
            data,
            pd.DataFrame(columns=["变量", "机构", "月份", "PSI值"]),
            pd.DataFrame(columns=["变量", "处理原因"]),
        )
    ...
    for org in all_orgs:
        ...
        for var in all_vars:
            ...
            psi_detail_list.append(
                {"机构": org, "变量": var, "月份": months[0], "PSI值": 0.0}
            )
            ...
            for i in range(1, len(months)):
                ...
                if len(var_month_data) > 0:
                    ...
                    psi_value = 0.0
                psi_detail_list.append(
                    {"机构": org, "变量": var, "月份": month, "PSI值": psi_value}
                )
    ...
    org_summary = (
        valid_psi.groupby(["机构", "变量"])
        .agg(不稳定月份数=("不稳定", "sum"), 总月份数=("变量", "count"))
        .reset_index()
    )
    ...
    org_count = len(orgs)
    channel_summary = (
        org_summary.groupby("变量")
        .apply(
            lambda x: pd.Series(
                {"机构数": org_count, "不稳定机构数": x["是否不稳定"].sum()}
            )
        )
        .reset_index()
    )
    ...
    channel_summary["处理原因"] = channel_summary.apply(
        lambda x: (
            f'PSI unstable in {x["不稳定机构数"]} organizations' if x["需处理"] else ""
        ),
        axis=1,
    )
    ...
    for var in org_summary["变量"].unique():
        var_orgs = org_summary[
            (org_summary["变量"] == var) & (org_summary["是否不稳定"] == True)
        ]["机构"].tolist()
    ...
    psi_process["不稳定机构"] = psi_process["变量"].apply(
...
def iv_distribution_by_org(
    iv_detail: pd.DataFrame,
    oos_orgs: list = None,
    iv_bins: list = [0, 0.02, 0.05, 0.1, float("inf")],
) -> pd.DataFrame:
...
def psi_distribution_by_org(
    psi_detail: pd.DataFrame,
    oos_orgs: list = None,
    psi_bins: list = [0, 0.05, 0.1, float("inf")],
) -> pd.DataFrame:
...
def value_ratio_distribution_by_org(
    data: pd.DataFrame,
    features: List[str],
    oos_orgs: list = None,
    value_bins: list = [0, 0.15, 0.35, 0.65, 0.95, 1.0],
) -> pd.DataFrame:
...
def calculate_iv_by_org(
    data: pd.DataFrame, features: List[str], n_jobs: int = 4
    ...
    for org in orgs:
        org_data = data[data["new_org"] == org]
        org_iv = calculate_iv(
            org_data, features, n_jobs=1
        )  # Single process for single organization
    ...
    iv_by_org = (
        pd.concat(iv_by_org, ignore_index=True)
        if iv_by_org
        else pd.DataFrame(columns=["变量", "IV", "机构", "类型"])
    )

    return iv_by_org, iv_overall


def calculate_psi_detail(
    data: pd.DataFrame,
    features: List[str],
    max_psi: float = 0.1,
    min_months_unstable: int = 3,
    min_sample: int = 100,
    n_jobs: int = 4,
) -> tuple:
    ...
    for org in orgs:
        ...
        for i in range(len(months) - 1):
            ...
            for f in features:
                tasks.append(
                    (org, train_month, test_month, train_n, test_n, f, data, min_sample)
                )
    ...
    if len(psi_df) == 0:
        return (
            data,
            pd.DataFrame(columns=["变量", "机构数", "不稳定机构数", "原因"]),
            pd.DataFrame(
                columns=["变量", "机构数", "不稳定机构数", "是否剔除", "去除条件"]
            ),
        )
    ...
    channel_summary["需剔除"] = channel_summary["不稳定机构数"] > (
        channel_summary["机构数"] / 3
    )
    channel_summary["是否剔除"] = channel_summary["需剔除"].astype(int)
    channel_summary["去除条件"] = channel_summary.apply(
        lambda x: (
            f"More than 1/3 of {org_count} organizations have PSI>{max_psi} for {min_months_unstable} consecutive months"
            if x["需剔除"]
            else ""
        ),
        axis=1,
    ...
    return (
        data,
        dropped[["变量", "机构数", "不稳定机构数", "原因"]],
        channel_summary[["变量", "机构数", "不稳定机构数", "是否剔除", "去除条件"]],
    )
...
def export_cleaning_report(
    ...
    ws["A3"] = (
        "Note: Each filtering step is executed independently, data is not deleted, only statistics of features that do not meet conditions are recorded"
    )
            ...
            if df is not None and len(df) > 0:
                if name == "分离OOS数据":
                    ...
                    if "变量" in df.columns and "数量" in df.columns:
                        ...
                        oos_count = (
                            df[df["变量"] == "OOS样本"]["数量"].values[0]
                            if len(df[df["变量"] == "OOS样本"]) > 0
                            else 0
                        )
                        model_count = (
                            df[df["变量"] == "建模样本"]["数量"].values[0]
                            if len(df[df["变量"] == "建模样本"]) > 0
                            else 0
                        )
                        result = (
                            f"OOS samples {oos_count}, modeling samples {model_count}"
                        )
                    else:
    ...
    for name, df in steps:
        if (
            name in filter_steps
            and df is not None
            and len(df) > 0
            and "变量" in df.columns
        ):
    ...
    ws.cell(
        4 + final_step_num,
        3,
        f"Total dropped {len(all_dropped_vars)} features (union of dropped features from each step)",
    )
    ws.cell(
        4 + final_step_num,
        4,
        "Each step is executed independently, final dropped features are the union of dropped features from each step",
    )
