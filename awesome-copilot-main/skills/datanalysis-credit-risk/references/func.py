...
def get_dataset(
    ...
    data.drop(
        columns=[c for c in data.columns if data[c].nunique() <= 1], errors="ignore"
    )

    # Rename columns
    data.rename(
        columns={
            date_colName: "new_date",
            y_colName: "new_target",
            org_colName: "new_org",
        },
        inplace=True,
    )
    data["new_date"] = (
        data["new_date"].astype(str).str.replace("-", "", regex=False).str[:8]
    )
...
def org_analysis(data: pd.DataFrame, oos_orgs: List[str] = None) -> pd.DataFrame:
    ...
    """
    stat = (
        data.groupby(["new_org", "new_date_ym"])
        .agg(
    ...
    if oos_orgs and len(oos_orgs) > 0:
        stat["样本类型"] = stat["new_org"].apply(
            lambda x: "贷外" if x in oos_orgs else "建模"
        )
    else:
        stat["样本类型"] = "建模"
...
def missing_check(
    data: pd.DataFrame, channel: Dict[str, List[str]] = None
...
def calculate_iv(
    ...
    def _calc_iv(f):
        try:
            ...
            data_temp["x_bin"] = c.fit_transform(
                X=data_temp["x"],
                y=data_temp["y"],
                method="dt",
                n_bins=5,
    ...
    results = Parallel(n_jobs=n_jobs, verbose=0)(delayed(_calc_iv)(f) for f in features)
