...
OUTPUT_DIR = _get_path_input(
    "Please enter output directory OUTPUT_DIR", default_output_dir
)
...
KEY_COLS = _get_list_input(
    "Please enter primary key column names in data (multiple columns separated by comma or space)",
    "record_id",
)
...
try:
    oos_input = input(
        "Please enter out-of-sample organization list, comma separated (press Enter to use default list):"
    ).strip()
except Exception:
    oos_input = ""
if oos_input:
...
print(
    f"   Organization count: {data['new_org'].nunique()}, Month count: {data['new_date_ym'].nunique()}"
)
...
oos_info = pd.DataFrame(
    {"变量": ["OOS样本", "建模样本"], "数量": [oos_data.shape[0], data.shape[0]]}
)
...
step_start = time.time()
data_filtered, abnormal_ym = drop_abnormal_ym(
    data.copy(),
...
print(
    f"   Parameters: min_ym_bad_sample={params['min_ym_bad_sample']}, min_ym_sample={params['min_ym_sample']}"
)
...
print(
    f"   Organization count: {len(miss_detail.columns) - 2}"
)  # Subtract '变量' and '整体' columns
...
data_miss, dropped_miss = drop_highmiss_features(
    data.copy(), miss_channel, threshold=params["missing_ratio"]
)
...
params["max_org_threshold"] = int(
    get_user_input("Maximum tolerated low IV organization count", 2, int)
)
step_start = time.time()
...
print(
    f"   Parameters: overall_iv_threshold={params['overall_iv_threshold']}, org_iv_threshold={params['org_iv_threshold']}, max_org_threshold={params['max_org_threshold']}"
)
if len(iv_process) > 0:
...
print(
    f"   Parameters: psi_threshold={params['psi_threshold']}, max_months_ratio={params['max_months_ratio']:.2f}, max_orgs={params['max_orgs']}"
)
...
print(
    f"   Parameters: n_estimators={params['n_estimators']}, max_depth={params['max_depth']}, gain_threshold={params['gain_threshold']}"
)
...
params["top_n_keep"] = int(
    get_user_input("Keep top N features by original gain ranking", 20, int)
)
step_start = time.time()
# Get feature list (use all features)
features = [c for c in data.columns if c.startswith("i_")]
...
value_ratio_distribution = value_ratio_distribution_by_org(
    data, features_for_value_ratio, oos_orgs=OOS_ORGS
)
print(
    f"   Value ratio distribution statistics: {len(value_ratio_distribution)} records"
)
