...
class CrewAgent:
    ...
    def llm_reply(self, message, context=None):
        ...
        endpoint = self.llm_config.get('endpoint', 'https://api.openai.com/v1/chat/completions')
...
def load_session(session_id):
    ...
    parser.add_argument('--persona', type=str, help='Agent persona (e.g. Captain, Doctor)')
    ...
    while True:
        try:
            ...
            if len(session_state['history']) > session_state['context_window']:
                session_state['history'] = session_state['history'][-session_state['context_window']:]
