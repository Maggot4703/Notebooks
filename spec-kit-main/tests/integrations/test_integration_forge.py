...
class TestForgeCommandNameFormatter:
    ...
    def test_extension_command_name(self):
        ...
        assert (
            format_forge_command_name("my-extension.example")
            == "speckit-my-extension-example"
        )
    ...
    def test_name_with_hyphens_preserved(self):
        ...
        assert (
            format_forge_command_name("speckit.my-ext.test-cmd")
            == "speckit-my-ext-test-cmd"
        )

    def test_alias_formatting(self):
        """Test formatting alias names."""
        assert (
            format_forge_command_name("speckit.my-extension.example-short")
            == "speckit-my-extension-example-short"
        )
...
class TestForgeIntegration:
    ...
    def test_setup_creates_md_files(self, tmp_path):
        ...
        command_files = [
            f for f in created if f.parent == tmp_path / ".forge" / "commands"
        ]
    ...
    def test_directory_structure(self, tmp_path):
        ...
        actual_commands = {
            f.name.removeprefix("speckit.").removesuffix(".md") for f in command_files
        }
    ...
    def test_templates_are_processed(self, tmp_path):
        ...
        for cmd_file in commands_dir.glob("speckit.*.md"):
            ...
            ), f"{cmd_file.name} has unprocessed {{SCRIPT}}"
            assert (
                "__AGENT__" not in content
            ), f"{cmd_file.name} has unprocessed __AGENT__"
            ...
            assert (
                "$ARGUMENTS" not in content
            ), f"{cmd_file.name} has unprocessed $ARGUMENTS"
    ...
    def test_forge_specific_transformations(self, tmp_path):
        ...
        for cmd_file in commands_dir.glob("speckit.*.md"):
            ...
            assert (
                "handoffs" not in frontmatter
            ), f"{cmd_file.name} has unstripped 'handoffs' key in frontmatter"
...
class TestForgeCommandRegistrar:
    ...
    def test_registrar_formats_extension_command_names_for_forge(self, tmp_path):
        ...
        registrar = CommandRegistrar()
        commands = [
            {"name": "speckit.my-extension.example", "file": "commands/example.md"}
        ]

        registered = registrar.register_commands(
            "forge", commands, "test-extension", ext_dir, tmp_path
        )
    ...
    def test_registrar_formats_alias_names_for_forge(self, tmp_path):
        ...
        assert (
            "name" in frontmatter
        ), "name field should be injected in alias frontmatter"
    ...
    def test_registrar_does_not_affect_other_agents(self, tmp_path):
        ...
        registrar.register_commands(
            "windsurf", commands, "test-extension", ext_dir, tmp_path
        )
        ...
        windsurf_cmd = (
            tmp_path / ".windsurf" / "workflows" / "speckit.my-extension.example.md"
        )
